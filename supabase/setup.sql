...
-- Saved/favorited cards (user side only; claude side reserved for future use)
CREATE TABLE crosstalk_favorites (
  id serial PRIMARY KEY,
  pair_id integer NOT NULL,
  side text NOT NULL CHECK (side IN ('claude', 'user')),
  created_at timestamptz DEFAULT now(),
  UNIQUE (pair_id, side)
);

-- Row Level Security (open for simplicity — personal tool, access via Edge Function service role)
...
ALTER TABLE crosstalk_favorites ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "public_all" ON crosstalk_favorites FOR ALL USING (true) WITH CHECK (true);
